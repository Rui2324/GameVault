    ...
    <div className="h-full flex flex-col gap-6">
      ...
      <RetroCard color="rose" className="p-6 relative overflow-hidden">
        ...
        <div className="relative flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2 text-rose-400 text-sm font-bold uppercase tracking-widest mb-2">
              <span className="inline-block w-3 h-3 bg-rose-400 animate-pulse" />
              Lista de Desejos
            </div>
        ...
        <div className="relative mt-4 flex gap-4">
          <div className="border-2 border-yellow-400/50 bg-yellow-400/10 px-4 py-2">
            <div className="text-2xl font-black text-yellow-400">{wishlist.length}</div>
            <div className="text-[11px] text-slate-400 font-bold uppercase">jogos na lista</div>
          </div>
        </div>
        ...
        <div className="absolute bottom-4 right-6 w-3 h-3 bg-rose-500" />
      </RetroCard>
      ...
      <RetroCard color="fuchsia" className="flex-1 p-5">
        {loading ? (
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div key={i} className="animate-pulse border-2 border-slate-700 bg-slate-800 p-4">
                <div className="flex gap-3">
                  <div className="w-16 h-24 bg-slate-700" />
                  <div className="flex-1 space-y-2">
                    <div className="h-4 bg-slate-700 w-3/4" />
                    <div className="h-3 bg-slate-700 w-1/2" />
                    <div className="h-3 bg-slate-700 w-2/3" />
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="flex items-center justify-center h-40 text-rose-400 text-sm border-2 border-rose-500/50 bg-rose-500/10">
            <span className="mr-2">⚠️</span> {erro}
          </div>
        ) : wishlist.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-slate-400">
            <div className="text-6xl mb-4">🎮</div>
            <span className="text-lg font-bold text-white">A tua wishlist está vazia</span>
            <span className="text-sm mt-1">Adiciona jogos que queres jogar no futuro!</span>
            <RetroButton color="rose" onClick={abrirModal} className="mt-4">
              ✨ Explorar jogos
            </RetroButton>
          </div>
        ) : (
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {wishlist.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="group relative flex gap-4 border-2 border-cyan-400/50 bg-slate-800 p-4 hover:border-cyan-400 hover:bg-slate-800/80 transition-all duration-200"
                >
                  {/* Badge decorativo */}
                  ...
                  >
                    {capa ? (
                      <img
                        src={capa}
                        alt={titulo}
                        className="w-16 h-24 object-cover border-2 border-fuchsia-500/50 group-hover/img:border-fuchsia-500 transition-all"
                      />
                    ) : (
                      <div className="w-16 h-24 border-2 border-fuchsia-500/50 bg-slate-700 flex items-center justify-center text-2xl group-hover/img:border-fuchsia-500 transition-all">
                        🎮
                      </div>
                    )}
                  </button>
                  <div className="flex flex-col justify-between flex-1 min-w-0">
                    <div>
                      ...
                      >
                        <h3 className="text-sm font-bold text-white line-clamp-2 hover:text-cyan-400 transition-colors">
                          {titulo}
                        </h3>
                      </button>
                    ...
                    <div className="flex gap-2 mt-3">
                      ...
                      <button
                        ...
                        className="border-2 border-rose-500/50 bg-rose-500/20 px-3 py-1 text-[11px] font-bold text-rose-400 hover:bg-rose-500 hover:text-white disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                      >
      ...
      {mostrarModal && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/80">
          <RetroCard color="cyan" className="w-full max-w-3xl p-6 relative">
            {/* Grid pattern */}
            ...
            <div className="relative">
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  onClick={fecharModal}
                  className="w-8 h-8 border-2 border-slate-600 bg-slate-800 hover:border-rose-500 hover:bg-rose-500/20 text-slate-400 hover:text-rose-400 transition-all flex items-center justify-center text-lg font-bold"
                >
                  ×
                </button>
              </div>
              ...
              >
                <div className="flex-1 relative">
                  ...
                  <input
                    ...
                    className="w-full border-2 border-slate-600 bg-slate-800 pl-10 pr-4 py-3 text-sm text-slate-100 placeholder:text-slate-500 focus:outline-none focus:border-fuchsia-500 transition-colors"
                  />
                </div>
                <RetroButton
                  type="submit"
              ...
              {erroPesquisa && (
                <div className="mb-4 text-sm text-rose-400 border-2 border-rose-500/50 bg-rose-500/10 px-4 py-2">
                  ⚠️ {erroPesquisa}
                </div>
              )}
              ...
              <div className="max-h-[380px] overflow-y-auto space-y-2 pr-2">
                {loadingPesquisa ? (
                  <div className="space-y-2">
                    {[1, 2, 3].map((i) => (
                      <div key={i} className="animate-pulse flex items-center gap-3 border-2 border-slate-700 bg-slate-800 px-4 py-3">
                        <div className="w-12 h-16 bg-slate-700" />
                        <div className="flex-1 space-y-2">
                          <div className="h-4 bg-slate-700 w-2/3" />
                          <div className="h-3 bg-slate-700 w-1/2" />
                        </div>
                      </div>
                    ))}
                  ...
                  resultadosExternos.map((jogo) => (
                    <div
                      ...
                      className="flex items-center justify-between gap-4 border-2 border-slate-700 bg-slate-800 px-4 py-3 hover:border-fuchsia-500/50 transition-all"
                    >
                      <div className="flex items-center gap-3">
                        {jogo.cover_url ? (
                          <img
                            src={jogo.cover_url}
                            alt={jogo.title}
                            className="w-12 h-16 object-cover border-2 border-slate-600"
                          />
                        ) : (
                          <div className="w-12 h-16 border-2 border-slate-600 bg-slate-700 flex items-center justify-center text-xl">
                            🎮
                          </div>
                        )}
                      ...
                      >
                        {aImportarId === jogo.external_id ? "⏳ A adicionar..." : "💝 Adicionar"}
                      </RetroButton>
                    </div>
                  ))
                )}
